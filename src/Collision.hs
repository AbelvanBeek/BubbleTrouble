...
checkNoRoofCollision (EnemyObjects (Ball (ObjectInfo _ (_,vy) (_,y) (Size w _) ))) | ((vy + y) > (350 - adjustsize)) = False
                                                                                   | otherwise = True
...
splitb (EnemyObjects (Ball (ObjectInfo a _ c (Size w h) ))) | w > 0.2 = ((EnemyObjects (Ball (ObjectInfo a ((-ballSpeed), ballHitY - 2 * w) c halfsize))) 
...
collision (PlayerObjects (Arrow (ObjectInfo _ (avx,avy) (ax, ay) _))) (EnemyObjects (Ball (ObjectInfo _ (bvx,bvy) (bx, by) (Size w _))))
            ...
            | otherwise = False
                  where adjustsize = halfBallSprite * w
collision (Player (P1 (PlayerInfo (ObjectInfo _ (pvx,pvy) (px,py) _) _ _ _))) (EnemyObjects (Ball (ObjectInfo _ (bvx,bvy) (bx, by) (Size w _))))
...
collision (Player (P2 (PlayerInfo (ObjectInfo _ (pvx,pvy) (px,py) _) _ _ _))) (EnemyObjects (Ball (ObjectInfo _ (bvx,bvy) (bx, by) (Size w _))))
            | (sqrt ((xdistance*xdistance) + (ydistance * ydistance) ) < halfPlayerSprite + adjustsize) = True
            | otherwise = False
                  where adjustsize = halfBallSprite * w
                        xdistance = abs ((px + pvx) - (bx + bvx))
...
checkPlayerCollided (Level p1@(Player (P1 (PlayerInfo d1 o1 n1 p1life))) _ p2@(Player (P2 (PlayerInfo d2 o2 n2 p2life))) _ enemies _ _ _)
...
checkSideCollision (ObjectInfo _ (vx,vy) (px, py) (Size w _))     | (px + vx) < ((-halfScreenWidth) + adjustsize) = True 
                                                                  | (px + vx) > (halfScreenWidth - adjustsize) = True
                                                                  | otherwise = False
                                                                        where adjustsize = halfBallSprite * w
--check collision with the floor of the map
checkFloorCollision (ObjectInfo _ (vx,vy) (px, py) (Size w _))    | (py + vy) < (-halfScreenHeight + adjustsize) = True
                                                                  | otherwise = False
                                                                        where adjustsize = halfBallSprite * w